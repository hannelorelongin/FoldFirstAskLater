...
def run_alphafold_advanced_complex(seq, jobname, save_dir, use_templates, num_runs_per_model, do_relax, max_recycles=3, tolerance=0, msa_mode='alphafold_default'):
    ...
    for perm_idx, query_sequences in enumerate(set(itertools.permutations(sequences))):
        ...
        fh = logging.FileHandler(filename=f'{out_dir}/{jobname}_info.log', mode='w')
        ...
        # write sequence coverage to log file
        logger.info('Sequence coverage: {}'.format((feature_dict['msa'] != 21).sum(0)))
        # remove intermediate directories
        if msa_mode.startswith('mmseqs2'):
            # store sequence identifiers first
            command = f"for i in $(find {out_dir}/{jobname}_*{'env' if use_env else ''}/ -name '*.a3m'); do cat $i | grep -a '>' | cut -f 1 | tail -n +2 >> {out_dir}/{jobname}_msa_ids.txt; done"
    ...
    for n, r in enumerate(rank):
        ...
        if (do_relax == 'best' and n == 0) or (do_relax == 'all'):
            try:
                ...
                relaxed_pdb_path = f'{out_dir}/{jobname}_relaxed.pdb'
