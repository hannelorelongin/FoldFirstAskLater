# original Phold database installation and validation code, now stripped from db.py and in a separate script
...
from foldfirst.utils.util import atomic_write_path, remove_directory
...
def install_database(
    ...
    if foldseek_gpu:
        if gpu_flag:
            ...
            foldseek_makepaddedseqdb(db_dir, "all_phold_structures")
...
def foldseek_makepaddedseqdb(db_dir: Path, db_name: str) -> None:
    ...
    db_search_gpu = Path(db_dir) / f"{db_name}_gpu"
    logdir = Path(db_dir) / "logdir"
    ...
    foldseek_makepaddedseqdb = ExternalTool(
        ...
        input=f"",
        output=f"",
        params=f"makepaddedseqdb {db_search} {db_search_gpu}",
        logdir=logdir,
    )
