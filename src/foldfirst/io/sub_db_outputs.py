...
# topfunction merge in foldfirst.results.phold_parsing.
...
class _SubDB:
    ...
    out_name: str               # output TSV name under phold_sub_db_tophits/

    def output_path(self, output: Path) -> Path:
        return Path(output) / "phold_sub_db_tophits" / self.out_name # check if this is correct when testing
...
def create_sub_db_outputs(merged_df: pl.DataFrame, database: Path, output: Path) -> bool:
    """Write per-sub-database TSVs to ``output/phold_sub_db_tophits/``.
    ...
    Args:
        ...
        output:    Output root; ``output/phold_sub_db_tophits/`` is created.
